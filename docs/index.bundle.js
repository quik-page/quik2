...
'</div>';
  ...
  var addon_dialog=new dialog({
    content:("<div class=\"addon-bar\"><h1>插件管理</h1><div class=\"r\"><div class=\"add-btn\">{{add-btn}}</div><div class=\"closeBtn\">{{close-btn}}</div></div></div><ul class=\"addon-root\"></ul>").replace('{{close-btn}}',util.getGoogleIcon('e5cd')).replace('{{add-btn}}',util.getGoogleIcon('e145')),
    mobileShowtype:dialog.SHOW_TYPE_FULLSCREEN,
    class:"addon-dialog"
  });
