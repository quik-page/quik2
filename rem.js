...
return posix;
  ...
  function gse(sss){
    if(sss.getAttribute('type')=='text/rem'){
      if(sss.hasAttribute('src')){
        qjs(sss.getAttribute('src'),function(code){
          eval(code);
        });
      }else{
        ijs(sss.innerHTML,function(code){
          eval(code);
        });
      } 
    }
 ...
 * @param {String} src 
 */
function _REQUIRE_(src){
  console.warn('注意！有_REQUIRE_方法未被替换，失效的js:'+src);
}